  ...
  try {
    ...
    try {
      ...
      (usersList || []).forEach(user => {
        if (user.location) {
          const locationKey = typeof user.location === 'string' ? user.location : user.location.country || 'Unknown';
          demographics.byLocation[locationKey] = (demographics.byLocation[locationKey] || 0) + 1;
        }
      });
